...
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-gray-800/80"
  ...
  <div id="boost-context-menu"
    ...
    <div class="rounded-2xl border overflow-hidden shadow-2xl shadow-emerald-500/10"
      ...
      <a href="<?= $base ?>/tickets"
         onclick="closeBoostMenu()"
         class="flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors border-b border-[#1a2332]">
        <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style="background:rgba(0,212,255,0.08)">
        ...
        <div>
          ...
          <div class="text-[10px] text-gray-500">Ouvrir un ticket d'assistance</div>
        </div>
      </a>
      <a href="<?= $base ?>/rewards"
         onclick="closeBoostMenu()"
         class="flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors">
        <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style="background:rgba(6,182,212,0.1)">
        ...
        <div>
          <div class="text-xs font-bold text-white">Fidélité & Cashback</div>
          <div class="text-[10px] text-gray-500">Consulter et échanger mes points</div>
        </div>
      </a>
      <?php endif; ?>
      <?php if ($isAdmin): ?>
      <?php
        try {
          $pdoNav = App\Core\Database::getInstance();
          $openTicketsNav = (int)$pdoNav->query("SELECT COUNT(*) FROM support_tickets WHERE status = 'open'")->fetchColumn();
        } catch (\Throwable $e) { $openTicketsNav = 0; }
      ?>
      ...
      <a href="<?= $base ?>/admin/tickets"
        ...
        <div class="flex-1">
          <div class="text-xs font-bold text-white flex items-center gap-2">
            Tickets Support
            <?php if ($openTicketsNav > 0): ?>
              <span class="text-[9px] font-black px-1.5 py-0.5 rounded-full" style="background:rgba(251,191,36,0.2);color:#fbbf24"><?= $openTicketsNav ?></span>
            <?php endif; ?>
          </div>
          <div class="text-[10px] text-gray-500">Répondre aux demandes clients</div>
        </div>
      </a>
  ...
  <div class="grid grid-cols-5 items-end justify-items-center h-16 px-1">
    ...
    <?php if ($isActive('/dashboard') || $path === '/'): ?>
    <button onclick="openRechargeModal()"
       class="relative flex flex-col items-center justify-center gap-1 w-full h-full transition-all active:scale-90 text-gray-500 hover:text-gray-300">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
